...
def evaluate_metrics(y_true, y_pred, labels=["Anth", "Bio", "Geo", "Sil"]):
    f1s = []
    for col in labels:
...
def bootstrap_ci(y_true, y_pred, n_bootstraps=1000, ci=95, labels=["Anth", "Bio", "Geo", "Sil"]):
    np.random.seed(42)
...
print("Max Prob F1: ", f1_score(df_max_prob, pred, average="macro"))
...
pred = pred.groupby("filename").apply(
    lambda x: pd.Series({
        "Anth": (x["Anth"] > .5).sum() >= 1, # 12, # 5, # & ((x["Bio"] > .1).sum() <= 35) & ((x["Geo"] > .1).sum() <= 15), #((x["Anth"] > .1).sum() >= 25) & ((x["Bio"] > .1).sum() <= 35) & ((x["Geo"] > .1).sum() <= 15)
        "Bio": (x["Bio"] > .5).sum() >= 1, # 10, # 2, # 45
        "Geo": (x["Geo"] > .5).sum() >= 1, # 10, # 3, # 45
        "Sil": (x["Sil"] > threshold).sum() >= 55, # 12, # 55, # 55
    })
)
...
for idx, (version_name, df) in enumerate([
    ...
    print("Whole test set: ", f1_score(df, pred, average="macro"))
    print("subset: ", f1_score(df.loc[newly_annotated_files], pred.loc[newly_annotated_files], average="macro"))
    print(f"Total macro Recall: {version_name}")
    print("Whole test set: ", recall_score(df, pred, average="macro"))
    print("subset: ", recall_score(df.loc[newly_annotated_files], pred.loc[newly_annotated_files], average="macro"))
    print(f"Total macro Precision: {version_name}")
    print("Whole test set: ", precision_score(df, pred, average="macro"))
    print("subset: ", precision_score(df.loc[newly_annotated_files], pred.loc[newly_annotated_files], average="macro"))
...
df_f1 = pd.DataFrame(f1_data, index=["Anth", "Bio", "Geo", "Sil"]).T


# Save to CSV
# df_f1.to_csv(f'./analysis/f1_score_summary_{version}.csv')
